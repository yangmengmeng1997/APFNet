...
'''
   ...
   You can use it for testing the stage1.
'''
...
class MDNet(nn.Module):
    def __init__(self, model_path=None, K=1):
        ...
        self.parallel1 = nn.Sequential(OrderedDict([
                ('parallel1_conv1',nn.Sequential(nn.Conv2d(3, 32, kernel_size=5, stride=2),nn.ReLU(),)),  #52
                ('parallel1_conv2',nn.Sequential(nn.Conv2d(32, 96, kernel_size=4, stride=2)))]))   #25
        #Attribute fusion branch based on SKNet
    ...
    def load_model(self, model_path):
        ...
        try:
            ...
            #we frist test the FM barnch is valid for tracking? and next test all branches
            #we can add the orther branch for test ,You need change the branch model like OCC.....
            self.parallel1.load_state_dict(torch.load('./models/GTOT_FM.pth')['parallel1'])
            self.paralle1_skconv.load_state_dict(torch.load('./models/GTOT_FM.pth')['paralle1_skconv'])
            self.parallel2.load_state_dict(torch.load('./models/GTOT_FM.pth')['parallel2'])
            self.paralle2_skconv.load_state_dict(torch.load('./models/GTOT_FM.pth')['paralle2_skconv'])
            self.parallel3.load_state_dict(torch.load('./models/GTOT_FM.pth')['parallel3'])
            self.paralle3_skconv.load_state_dict(torch.load('./models/GTOT_FM.pth')['paralle3_skconv'])
