...
class MDNet(nn.Module):
    ...
    def Transformer_feature_layer2_vis(self,x):
        ...
        output = torch.bmm(affinity, w_v)   
        # output=output.permute(0,2,1)
        # output=self.transformer2_FFN[1](nn.Dropout(0.2)(F.relu(((self.transformer2_FFN[0](output))))))
        # output=output.permute(0,2,1)
        output=output.reshape(batch,dim,w,h)  
    ...
    def Transformer_feature_layer3_vis(self,x):
        ...
        output = torch.bmm(affinity, w_v)   
        # output=output.permute(0,2,1)
        # output=self.transformer3_FFN[1](nn.Dropout(0.2)(F.relu(((self.transformer3_FFN[0](output))))))
        # output=output.permute(0,2,1)
        output=output.reshape(batch,dim,w,h)  
